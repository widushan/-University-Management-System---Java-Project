...
public class UpdateSemesterPayment extends JFrame implements ActionListener {
    ...
    UpdateSemesterPayment() {
        ...
        JLabel heading = new JLabel("Update Semester Payments");
        ...
        update.setBounds(200, 900, 120, 30);
        ...
        if (ae.getSource() == search) {
            ...
            try {
                ...
                if (studentRs.next()) {
                    ...
                    String degree = studentRs.getString("degree");
                    ...
                    // Fetch previously added semester and status fields from the 'semesterpayments' table
                    String paymentQuery = "SELECT * FROM semesterpayments WHERE rollno = '" + rollno + "'";
                    ResultSet paymentRs = c.s.executeQuery(paymentQuery);

                    dynamicPanel.removeAll(); // Clear existing rows
                    semesterPaymentRows.clear();

                    while (paymentRs.next()) {
                        String semester = paymentRs.getString("semester");
                        String status = paymentRs.getString("status");
                        addSemesterPaymentRow(semester, status); // Add rows with pre-filled data
                    }
            ...
            if (rollno == null || rollno.trim().isEmpty()) {
                JOptionPane.showMessageDialog(null, "Please search and select a student before updating.");
                return;
            }
            ...
            try {
                ...
                // Delete existing records for the roll number
                String deleteQuery = "DELETE FROM semesterpayments WHERE rollno = '" + rollno + "'";
                ...
                for (SemesterPaymentRow row : semesterPaymentRows) {
                    ...
                    if (!semester.isEmpty() && !status.isEmpty()) {
                        String insertQuery = "INSERT INTO semesterpayments (rollno, semester, status) VALUES ('" + rollno + "', '" + semester + "', '" + status + "')";
                ...
                JOptionPane.showMessageDialog(null, "Semester Payments Updated Successfully!");
                setVisible(false); // Close the window after updating
            } catch (Exception ex) {
                ex.printStackTrace();
                JOptionPane.showMessageDialog(null, "Error occurred while updating payments.");
            }
        }
    ...
    private void addSemesterPaymentRow(String semester, String status) {
        JPanel rowPanel = new JPanel(new FlowLayout(FlowLayout.LEFT));
        JTextField semesterField = new JTextField(semester, 20);
        JTextField paymentField = new JTextField(status, 10);
        JButton removeBtn = new JButton("Remove");
